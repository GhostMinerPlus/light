use std::sync::Arc;

use actix_web::{web, HttpResponse, Responder};
use edge_lib::{data::DataManager, mem_table::MemTable, EdgeEngine};
use tokio::sync::Mutex;

use super::dto;

#[actix_web::post("/system/add_proxy")]
async fn add_proxy(
    global: web::Data<Arc<Mutex<MemTable>>>,
    web::Json(proxy): web::Json<dto::Proxy>,
) -> impl Responder {
    let mut edge_engine = EdgeEngine::new(DataManager::with_global((**global).clone()));

    // let mut proxies = ctx.proxy.lock().unwrap();
    // proxies.insert(proxy.path.clone(), proxy.url.clone());
    HttpResponse::Ok().finish()
}

#[actix_web::delete("/system/remove_proxy")]
pub async fn remove_proxy(
    global: web::Data<Arc<Mutex<MemTable>>>,
    web::Json(proxy): web::Json<dto::Proxy>,
) -> impl Responder {
    let mut edge_engine = EdgeEngine::new(DataManager::with_global((**global).clone()));

    // let mut proxies = ctx.proxy.lock().unwrap();
    // proxies.remove(&proxy.path);
    HttpResponse::Ok().finish()
}

#[actix_web::get("/system/list_proxies")]
pub async fn list_proxies(global: web::Data<Arc<Mutex<MemTable>>>,) -> impl Responder {
    let mut edge_engine = EdgeEngine::new(DataManager::with_global((**global).clone()));

    // let proxies = ctx.proxy.lock().unwrap();

    // let mut list = Vec::with_capacity(proxies.len());
    // for proxy in &*proxies {
    //     list.push(dto::Proxy {
    //         path: proxy.0.clone(),
    //         url: proxy.1.clone(),
    //     });
    // }
    HttpResponse::Ok()
        .content_type("application/json")
        .body("serde_json::to_string(&list).unwrap()")
}
